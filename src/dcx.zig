...
export fn parseHeader(bytes: []u8) CHeader {
...
export fn parseData(
    ...
    reader.readNoEof(compressed_data) catch return 1;

    if (std.mem.eql(u8, &header.format, "NONE")) {
        if (header.compressedSize != header.uncompressedSize) {
            return 1;
        }
        @memcpy(buffer, compressed_data);
        return 0;
    } else if (std.mem.eql(u8, &header.format, "DFLT")) {
        var stream = std.io.fixedBufferStream(compressed_data);
        var decompressor = std.compress.zlib.decompressor(stream.reader());
        ...
        decompressor.reader().readNoEof(decompressed) catch return 1;

        @memcpy(buffer, decompressed);
        return 0;
    } else return 1;
}
...
export fn parse(
    ...
    const header = parseHeader(bytes[0..@sizeOf(CHeader)]);
    const data = parseData(header, bytes[@sizeOf(CHeader)..], buffer);
